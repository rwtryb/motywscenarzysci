...
function my_pre_save_post( $post_id ) {
    ...
    if( $post_id != 'new' & $post_id != 'projekt' & $post_id != 'sesja' & $post_id != 'dyzur')
    {
        return $post_id;
    }
    ...
    // Create a new projekt
		if ($post_id == 'projekt') {
          $post = array(
        'post_status'  => 'publish',
        'post_title'  => $_POST['fields']['field_586aca9beb4a2'],
        'post_type'  => 'projekt',
    );
    }
  	...
  	if ($post_id == 'sesja') {
          $post = array(
        'post_status'  => 'publish',
        'post_title'  => $_POST['fields']['field_5877db85a9eab'],
        'post_type'  => 'sesja',
    );
    }
  	...
  	if ($post_id == 'dyzur') {
          $post = array(
        'post_status'  => 'publish',
        'post_title'  => $_POST['fields']['field_59cb99980e15f'],
        'post_type'  => 'dyzur',
    );
    }
...
if ( ! function_exists( 'auto_custom_class_add_for_media_attchment' ) ) :
    ...
    function auto_custom_class_add_for_media_attchment( $html, $id ) {
        ...
        if ( $mime_type == 'application/pdf' ) {
            ...
            $html = '<a class="fileuploadclass" target="_blank" href="'. $src .'">'. $attachment->post_title .'</a>';
        }
